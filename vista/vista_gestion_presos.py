...
def crear_vista_presos(lista_presos, on_crear_preso_handler, on_refrescar_handler):
    """
    Crea la tarjeta de gestión de presos.
    Recibe la lista actual de presos y las funciones del controlador.
    """
    ...
    if not lista_presos:
        ...
        for preso in lista_presos:
            prisoner_list.controls.append(
                ft.Container(
                    content=ft.Text(f"ID: {preso['id']} - {preso['nombre']} ({preso['delito']})", color=COLORS['text']),
    ...
    prisoner_list_container = ft.Container(
        ...
        bgcolor=COLORS['glass'], 
        height=150, # Aumentamos altura
        padding=10
    )
    
    prisoner_card = ft.Container(
        bgcolor=COLORS['card'], 
        border=ft.border.all(2, COLORS['glass']), # Borde más sutil
        expand=True, 
        padding=12,
        content=ft.Column([
            prisoner_header,
            campo_nombre_preso, # Añadimos el campo de texto
            ft.Divider(height=5, color="transparent"),
            prisoner_list_container
        ])
    )
