...
def crear_vista_usuarios(rol_actual, lista_usuarios, on_crear_usuario_handler):
    ...
    new_user_pass = ft.TextField(label="Contraseña", password=True, bgcolor=COLORS['glass'], color=COLORS['text'],
                                 border_color=COLORS['glass'], height=40, text_size=12, content_padding=10)
    new_user_role = ft.Dropdown(value="policia",
                                options=[ft.dropdown.Option("inspector"), ft.dropdown.Option("policia")],
    ...
    if not lista_usuarios:
        ...
        for user in lista_usuarios:
            user_list.controls.append(ft.Text(f"· {user['user']} ({user['rol']})", color=COLORS['text']))

    # Lista con altura limitada pero expandible dentro de lo razonable
    user_list_container = ft.Container(content=user_list, bgcolor=COLORS['glass'], padding=10, border_radius=5,
                                       expand=True)

    # Tarjeta Principal: expand=True para llenar su mitad de la pantalla
    user_card = ft.Container(
        bgcolor=COLORS['card'], border=ft.border.all(2, COLORS['glass']), padding=12,
        visible=(rol_actual == 'comisario'),
        expand=True,
        content=ft.Column([user_header, user_form, ft.Divider(height=5, color="transparent"), user_list_container])
    )
